...
#Empties current file and then writes new IP's from ipv4 and ipv6 text links to the file.
#> /etc/csf/cloudflare.allow ; for x in $(curl https://www.cloudflare.com/{ips-v4,ips-v6}); do echo "$x" | tee -a /etc/csf/cloudflare.allow  ; done
